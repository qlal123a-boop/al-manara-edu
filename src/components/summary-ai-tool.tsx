...
export function SummaryAiTool() {
  const { user, refreshSubscription } = useAuthUser();
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (res.summary) {
        setSum(res.summary);
        if (user) {
           await supabase.rpc("increment_daily_usage", { _user_id: user.id, _action_type: "ai_generation" });
           refreshSubscription();
        }
  ...
  return (
    <SubscriptionGuard actionType="ai_generation">
      <div>
